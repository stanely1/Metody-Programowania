...
(define-type-alias Storage (Listof (Location * Storable)))
...
(define (alloc-sto [sto : Storage] [u : Storable]) : (Location * Storage)
  (let ([next-free
         (if (empty? sto)
             ...
             (+ 1 (fst (first sto))))])
    (pair next-free
          (cons (pair next-free u) sto))))

(define (assoc [sto : Storage] [l : Location]) : Storable
  (type-case Storage sto
    [empty (error 'assoc "invalid location")]
    [(cons p s)
     (if (equal? (fst p) l)
         (snd p)
         (assoc s l))]))
...
(define (deref-sto [sto : Storage] [l : Location]) : Value
  (if (and (not (empty? sto)) (<= l (fst (first sto))))
      (type-case Storable (assoc sto l)
        [(valS v) v]
...
(define (update-sto [sto : Storage] [l : Location] [u : Storable]) : Storage
  (local
    ((define (walk sto)
       (if (= (fst (first sto)) l)
           (cons (pair l u) (rest sto))
           (cons (first sto) (walk (rest sto))))))
    (if (and (not (empty? sto)) (<= l (fst (first sto))))
        (walk sto)
        (error 'deref "unknown location"))))
