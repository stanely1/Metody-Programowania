...
(define-type Binding
  (bind [name : Symbol]
        [val : (-> Value)])) ;; zamiast wartosci trzymamy funkcje zwracające wartość
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE x e1 e2)
     (eval e2 (extend-env env x (lambda () (eval e1 env))))]
    [(lamE x b)
     (funV x b env)]
    [(appE e1 e2)
     (apply (eval e1 env) (lambda () (eval e2 env)))])) ;; nie liczymy e2 w tym miesjcu
